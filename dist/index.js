...
const getUserPreferences = async (userId) => {
    try {
        ...
        const response = await axios_1.default.get('http://localhost:3003/api/user/preferences', {
            headers: {
...
app.post('/api/upload', upload.single('video'), async (req, res) => {
    ...
    try {
        ...
        const playlistUrl = `https://s3.${userConfig.AWS_REGION}.amazonaws.com/${userConfig.S3_BUCKET_NAME}/output/${path_1.default.basename(file.filename, path_1.default.extname(file.filename))}/master.m3u8`;
        res.json({
            success: true,
...
const JOBS_STORAGE_FILE = path_1.default.join(__dirname, 'jobs_storage.json');
// Load previously stored jobs on startup
function loadJobsFromStorage() {
    try {
        if (fs_1.default.existsSync(JOBS_STORAGE_FILE)) {
            const jobsData = fs_1.default.readFileSync(JOBS_STORAGE_FILE, 'utf8');
...
function saveJobsToStorage() {
    try {
        ...
        fs_1.default.writeFileSync(JOBS_STORAGE_FILE, JSON.stringify(jobsObject, null, 2), 'utf8');
    }
    catch (error) {
...
function addSampleTranscodingLogs(jobId) {
    const job = activeJobs.get(jobId);
    if (!job)
        return;
...
async function execCommand(command) {
    const { stdout } = await (0, child_process_1.exec)(command);
    return stdout?.toString() || '';
}
// Function to monitor Docker container
async function monitorDockerContainer(jobId, containerId) {
    const job = activeJobs.get(jobId);
    if (!job)
        return;
    try {
...
app.get('/api/test-connection', async (req, res) => {
    ...
    try {
        ...
        try {
            await s3Client.send(new client_s3_1.ListBucketsCommand({}));
        ...
        try {
            ...
            const clusterResponse = await ecsClient.send(new client_ecs_1.DescribeClustersCommand({
        ...
        try {
            ...
            const subnetResponse = await ec2Client.send(new client_ec2_1.DescribeSubnetsCommand({
                SubnetIds: subnets
            }));
            details.ec2.subnets = {
                status: 'success',
            ...
            const sgResponse = await ec2Client.send(new client_ec2_1.DescribeSecurityGroupsCommand({
    ...
    (async () => {
        try {
            ...
            try {
                ...
                const s3ListResponse = await s3Client.send(new client_s3_1.ListObjectsV2Command(listParams));
                    ...
                    for (const prefix of s3ListResponse.CommonPrefixes) {
                        if (prefix.Prefix) {
                            ...
                            if (videoId && videoId.length > 5) {
                                ...
                                const existingJob = Array.from(activeJobs.values()).find(job => job.videoKey && job.videoKey.includes(videoId));
                                if (!existingJob) {
            ...
            try {
                ...
                const tasksResponse = await ecsClient.send(new client_ecs_1.ListTasksCommand(listTasksCommand));
                if (tasksResponse.taskArns && tasksResponse.taskArns.length > 0) {
                    ...
                    const taskDetails = await ecsClient.send(new client_ecs_1.DescribeTasksCommand(describeTasksCommand));
                    // Process each task
...
async function importJobsFromAWS() {
    if (initialJobImportDone || !userConfig)
        return;
...
app.post('/api/create-test-job', (req, res) => {
    try {
        // Generate a unique job ID
        const jobId = (0, uuid_1.v4)();
